...
export const CAPTION_PROMPT =
  "Describe this image factually for a knowledge-base index. Include: any visible text verbatim, chart axes and values, diagram structure (boxes/arrows/labels), key visual elements. If this is a flowchart, architecture diagram, sequence diagram, swimlane diagram, knowledge graph, full-page screenshot with multiple panels, or stitched workflow, prefer a compact graph-style description that preserves relationships: Nodes: key steps/components/actors; Edges: arrow direction, data/control flow, dependencies; Branches: conditions and alternate paths; Inputs/outputs: what each important node consumes or produces; Groups/swimlanes: roles, systems, phases, or clusters. Explain the overall flow, sequence, and relationships between parts, not just OCR text. Do NOT speculate or editorialize. Output plain text only — no markdown, no preamble."
...
export function buildCaptionPromptWithContext(
  ...
  return [
    ...
    "",
    "Now describe the image factually for a knowledge-base index. Include: any visible text verbatim, chart axes and values, diagram structure (boxes/arrows/labels), key visual elements. If this is a flowchart, architecture diagram, sequence diagram, swimlane diagram, knowledge graph, full-page screenshot with multiple panels, or stitched workflow, prefer a compact graph-style description that preserves relationships: Nodes: key steps/components/actors; Edges: arrow direction, data/control flow, dependencies; Branches: conditions and alternate paths; Inputs/outputs: what each important node consumes or produces; Groups/swimlanes: roles, systems, phases, or clusters. Explain the overall flow, sequence, and relationships between parts, not just OCR text. If the surrounding text contains a relevant figure number / caption / referent, incorporate that specifically. Do NOT invent details that aren't visible in the image or directly stated in the surrounding text. Output plain text only — no markdown, no preamble.",
  ].join("\n")
}
